...
#ifndef LIBPLUGWISE_CLOCKINFOREQUEST_HPP
#define LIBPLUGWISE_CLOCKINFOREQUEST_HPP 1

#include <request.hpp>
#include <connection.hpp>

namespace plugwise {
  class ClockInfoRequest : public Request {
    public:
      typedef std::tr1::shared_ptr<ClockInfoRequest> Ptr;
      ClockInfoRequest (std::string circle_plus_mac) :
        _circle_plus_mac(circle_plus_mac) {};

      void send(plugwise::Connection::Ptr con);
      virtual ~ClockInfoRequest() {};

    private:
      ClockInfoRequest (const ClockInfoRequest& original);
      ClockInfoRequest& operator= (const ClockInfoRequest& rhs);
