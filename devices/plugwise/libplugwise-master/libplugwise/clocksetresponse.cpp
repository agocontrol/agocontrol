...
std::string ClockSetResponse::str() {
  std::ostringstream oss;
  oss << "Clock Set Response: " << std::endl;
  ...
  oss << ", circle+ mac: " << _circle_plus_mac_addr;
  return oss.str();
}

void ClockSetResponse::parse_line2() {
  ...
  _circle_plus_mac_addr =(_line2.substr(12,16));
  // Skipping also the checksum (for now).
}


bool ClockSetResponse::req_successful() {
