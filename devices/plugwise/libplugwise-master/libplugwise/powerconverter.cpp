...
double PowerConverter::convertToWatt2(PowerInformationResponse::Ptr pir) {
  // for now, use the second interval data.
  uint32_t pulses = pir->getPulse1Sec();
  if ((pulses == 0) || (pulses == 0xffff))
    return 0.0;
  double value = pulses / 1.0;
  double result= 3600 *
    (
      (
        (pow(value + _off_noise, 2.0) * _gain_b
        )
        + (value + _off_noise) * _gain_a
      )
      + _off_tot
    );
  //return result;
