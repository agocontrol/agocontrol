...
#ifndef LIBPLUGWISE_CLOCKSETREQUEST_HPP
#define LIBPLUGWISE_CLOCKSETREQUEST_HPP 1

#include <request.hpp>
#include <connection.hpp>

namespace plugwise {
  class ClockSetRequest : public Request {
    public:
      typedef std::tr1::shared_ptr<ClockSetRequest> Ptr;
      ClockSetRequest (std::string circle_plus_mac) :
        _circle_plus_mac(circle_plus_mac) {};

      void send(plugwise::Connection::Ptr con);
      virtual ~ClockSetRequest() {};

    private:
      ClockSetRequest (const ClockSetRequest& original);
      ClockSetRequest& operator= (const ClockSetRequest& rhs);
