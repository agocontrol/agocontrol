...
#ifndef LIBPLUGWISE_INFOREQUEST_HPP
#define LIBPLUGWISE_INFOINITREQUEST_HPP 1
...
namespace plugwise {
  class InfoRequest : public Request {
    public:
      typedef std::tr1::shared_ptr<InfoRequest> Ptr;
      InfoRequest (std::string circle_mac) :
        _circle_mac(circle_mac) {};
      void send(plugwise::Connection::Ptr con);
      virtual ~InfoRequest() {};

    private:
      InfoRequest (const InfoRequest& original);
      InfoRequest& operator= (const InfoRequest& rhs);
